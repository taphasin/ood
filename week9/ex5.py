...
def check_binary_search_tree_(node, min_value=float('-inf'), max_value=float('inf')):
    if node is None:
        return True

    if not (min_value < abs(node.data) < max_value):
        return False

    # Recursively check left and right subtrees
    return (check_binary_search_tree_(node.left, min_value, abs(node.data)) and
            check_binary_search_tree_(node.right, abs(node.data), max_value))
